...
export const useCustomer = (initialCustomer = customerSchema)=>{
    ...
    const customerDataValidation = ()=>{
        ...
        const fields = [regex.name.test(customer['name']), regex.name.test(customer['surname'], regex.dni.test(customer['dni'], regex.tel.test(customer['tel']), regex.name.test(customer['address'])))]
